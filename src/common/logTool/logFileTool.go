...
func NewAJLogger(name string) (*ajLoggerT, error) {
	...
	var logPath string
	if paramTool.GetParam().LogParam.LogPath != "" {
		logPath = wd + commonTool.GetSysPthSep() + paramTool.GetParam().LogParam.LogPath
	} else {
...
func AutoManagementAJLogger(AJLogger *ajLoggerT) {
	...
	for {
		...
		for _, v := range files {
			...
			if err == nil && time.Now().Unix()-data.Unix() > int64(3600*24*paramTool.GetParam().LogParam.LogKeepDay) {
